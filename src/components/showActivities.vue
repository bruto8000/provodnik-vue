...
<div class="button z-depth-3" mx-2 style="width: 40px; padding: 0; border-radius: 50%;" @click="exportToExcel">
  <img src="../assets/EX.png" width="100%" style="border-radius: 50%;" alt=""></div>
</div>     
      ...
      <table id="allTable" v-show="false" v-if="table.created">
          ...
          <tr class="columns hover__bg" v-for="activity in activitiesFiltredPaginated" :key="activity.id" @click="openactivity(activity)">
      ...
      <div style="max-height: 75vh; overflow-y: auto; overflow-x: hidden;">
          ...
          <div class="columns hover__bg p-1" v-for="activity in activitiesFiltredPaginated" :key="activity.id" @click="openactivity(activity)">
...
export default {
  ...
  methods: {
    ...
    //   this.activities = this.activities.filter(activity=>!activity.deleted);
    //   console.log(this.activities.length)
    // }
  },
  watch: {
employees :{
  ...
  computed: {
    activitiesFiltred () {
      ...
      return this.activities.filter((v, i, arr) => {
